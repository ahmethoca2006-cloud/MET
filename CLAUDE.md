...
- **Layer groups/masks/smart objects** (adjustment layers *are* implemented, background-only — see Per-page layer stack above), **magnetic lasso**, **patch tool**, **curvature pen / path selection / direct selection**, **brush mirror/symmetry modes**, **Multi-Bubble mode** (TypeR) — declared as intended features (in tool lists, type unions, or SPEC) but not implemented. Each is a genuinely separate chunk of work. (Selection feather/expand/contract/add-subtract, liquify's `reconstruct` mode, and Crop *are* implemented — see Canvas engine above.)
