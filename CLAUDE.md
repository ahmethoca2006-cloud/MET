...
`studio_<chapterId>` is at **schema v4** (`STUDIO_SCHEMA_VERSION`; v2 added `autoWidth`/`letterSpacing`/`shadow`, v3 added `gradient`, v4 added `runs`). `migrateTextLayers` runs on *any* mismatched version and every backfill in it is an idempotent `??`, so one pass takes v1, v2 or v3 straight to current — don't restructure it into a per-step chain without keeping that property. This matters because the renderer/panel read nested fields (`text.shadow`, `text.gradient`) directly, so an un-backfilled layer throws on open.
