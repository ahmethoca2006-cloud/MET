...
import { Users, LogOut, ImagePlus, Save, Plus, Mail, Check, X, Crown, ShieldCheck } from 'lucide-react';
...
export function TeamsPanel() {
  const { session, isAdmin, signOut, updateProfile } = useTeamAuth();
    ...
    <GlassCard className="p-6 space-y-4 max-w-md">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-ink-muted">You are the admin</p>
        </div>
      </div>
          ...
          <div key={m.id} className="flex items-center justify-between gap-2 p-2 rounded-lg border border-hairline">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0">
                <p className="text-xs font-semibold text-ink truncate">{m.profile?.name || m.invited_email}</p>
                <p className="text-[10px] text-ink-faint truncate">{m.invited_email}</p>
              </div>
            </div>
            <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full shrink-0 ${m.status === 'active' ? 'bg-accent-soft text-accent' : 'bg-ink/5 text-ink-faint'}`}>
  ...
  if (membership) {
    return (
      <GlassCard className="p-6 space-y-4 max-w-md">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-2xl overflow-hidden bg-ink/5 border border-hairline shrink-0 flex items-center justify-center">
            {membership.team.logo ? <img src={membership.team.logo} alt={membership.team.name} className="w-full h-full object-cover" /> : <Users size={18} className="text-ink-faint" />}
          </div>
          <div>
            <h3 className="text-base font-semibold text-ink font-display">{membership.team.name}</h3>
            <p className="text-xs text-ink-muted flex items-center gap-1">
              {membership.role === 'leader' ? <><Crown size={12} className="text-accent" /> Leader</> : 'Member'}
            </p>
          </div>
        </div>
        <div className="space-y-2">
